<table class="list layout-attributes">
	<thead>
	<tr>
		<th class="t4"><?php echo __d('wasabi_cms', 'Field') ?></th>
		<th class="t12"><?php echo __d('wasabi_cms', 'Content') ?></th>
	</tr>
	</thead>
	<tbody>
	<?php if (!empty($this->request->data['CmsPageLayoutAttribute'])) {
		foreach ($this->request->data['CmsPageLayoutAttribute'] as $key => $attr) {
			?>
			<tr>
				<td><?php
...
<small><?php echo __d('wasabi_cms', 'Layout fields allow you to customize the layout for this page.') ?></small>